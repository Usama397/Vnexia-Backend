...
class MainController extends Controller
{
    ...
    public function guideCreate(Request $request)
    {
        ...
        $request->validate([
            ...
            'selectedIcon' => 'required|string',  // Validate the selected icon
        ]);
    
        // Create the safety guideline in the database
        $safety = Safety::create([
            "icon" => $request['selectedIcon'],  // Store the selected icon
            "Images" => "Car",  // Placeholder for image, modify as needed
            "title" => $request['title'],
            "description" => $request['description'],
            "CreatedBy" => $loginUser->id,
        ]);
